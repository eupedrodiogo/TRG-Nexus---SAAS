...
const Dashboard: React.FC<DashboardProps> = ({ isDarkMode = false, onOpenNotifications, privacyMode = false, therapist }) => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const data = await api.dashboard.data(therapist.id);
        if (data) {
          setStats(data.stats);
    ...
    <div className="space-y-6 animate-fade-in pb-20 md:pb-0">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 no-scrollbar w-full lg:w-auto">
          ...
          <button
            onClick={copyLink}
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 dark:bg-indigo-600 text-white rounded-xl shadow-lg shadow-indigo-500/20 hover:bg-indigo-700 transition-all active:scale-95 whitespace-nowrap"
